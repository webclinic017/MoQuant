...
def copy_basic_from_yesterday(result_list: list, store: mq_daily_store.MqDailyStore, ts_code, now_date):
    yesterday_date = format_delta(now_date, -1)
    for i in mq_daily_indicator_enum.extract_from_daily_list:
...
def calculate(ts_code: str, share_name: str, to_date: str = get_current_dt()):
    ...
    if len(last_mq_daily) > 0:
        from_date = format_delta(last_mq_daily[0].update_date, 1)
    else:
    ...
    quarter_store = mq_quarter_store.init_quarter_store_by_date(ts_code, format_delta(from_date, -360))
    daily_store = mq_daily_store.init_daily_store_by_date(ts_code, format_delta(from_date, -360))
...
def calculate_by_code(ts_code: str, to_date: str = get_current_dt()):
    session: Session = db_client.get_session()
    stock: MqStockMark = session.query(MqStockMark).filter(MqStockMark.ts_code == ts_code).one()
    session.close()
    calculate_and_insert(ts_code, stock.share_name, to_date)
